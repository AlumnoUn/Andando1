...
export class RecetasListComponent implements OnInit {
  ...
  // Si true muestra TODAS las recetas, para usuarios logueados
  showAll = false;

  // Esto es para saber que receta esta en favoritos del usuario (recetaId (string) -> favoritoId (string))
  favoritosMap = new Map<string, string>();

  ngOnInit() {
    // decidir modo por si el usuario ya estaba logueado al cargar la pagina
...
//   isFavorito(recetaId: number) {
//     return this.favoritosMap.has(recetaId);
//   }

//   async loadFavoritosIfLogged() {
//     const user = this.auth.getUser();
//     if (!user) {
//       this.favoritosMap.clear();
//       return;
...
//     this.favSvc.getFavoritosPorUsuario(user.id).subscribe(favs => {
//       this.favoritosMap.clear();
//       for (const f of favs) this.favoritosMap.set(f.recetaId, f.id);
//     });
//   }
...
//       this.favSvc.addFavorito(user.id, recetaId).subscribe({
//         next: f => this.favoritosMap.set(recetaId, f.id),
//         error: e => console.error('Error agregando favorito', e)
//       });
//     }
//   }
